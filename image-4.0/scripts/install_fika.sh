...
#
# Fika does NOT ship fika.jsonc — it is generated by the server on its first start, which
# happens after this script runs. So on a brand-new install the file is absent here and
# the patch cannot be applied; it lands from the second boot onward. That used to fail
# silently, leaving headless.profiles.amount at its default of 0 with nothing in the log
# to explain why no headless profile ever appeared. Say so instead.
if [ -n "$NUM_HEADLESS_PROFILES" ]; then
    if [ -f "$mod_dir/$config_rel" ]; then
        ...
        echo "NOTE: NUM_HEADLESS_PROFILES=${NUM_HEADLESS_PROFILES} not applied yet — Fika generates"
        echo "      ${config_rel} during its first server start, so it does not exist at install time."
        echo "      Restart the server once and it will be set. (Not needed if you are supplying your"
        echo "      own headless profile from the Fika Installer — the server counts what it finds.)"
    fi
fi
